...
SKYTEMPLE_RANDOMIZER_PYPROJECT_FILE = os.path.join(BASE_DIR, "..", "..", "pyproject.toml")
...
MANIFEST_TEMPLATE_FILE = "org.skytemple.Randomizer.yml.jinja"
OUT_MANIFEST_FILE = "org.skytemple.Randomizer.yml"
OUT_PIP_REQUIREMENTS_FILE = "requirements-skytemple-randomizer.json"
OUT_SKYTEMPLE_RUST_CARGO_FILE = "cargo-sources.json"
SKYTEMPLE_RANDOMIZER_GIT_URL = "https://github.com/SkyTemple/skytemple-randomizer.git"
...
def source_ref_skytemple_randomizer(git_tag: str | None) -> FlatpakSourceRef:
    ref: FlatpakSourceRef
    if git_tag is None:
        ref = {"type": "dir", "path": os.path.abspath(os.path.join(BASE_DIR, "..", ".."))}
    else:
        ref = {"type": "git", "url": SKYTEMPLE_RANDOMIZER_GIT_URL, "tag": git_tag}
    return print_and_return_ref("skytemple-randomizer", ref)
    ...
    with clone_repo_for_generator(skytemple_rust_req, SKYTEMPLE_RUST_GIT_URL) as path_skytemple_rust:
        with clone_repo_for_generator(explorerscript_req, EXPLORERSCRIPT_GIT_URL) as path_explorerscript:
            ...
            result = jinja_env.get_template("tmpl").render(
                skytemple_randomizer_ref=json.dumps(source_ref_skytemple_randomizer(tag)),
            ...
            run_pip_generator(reqs_file, os.path.join(out_dir, "requirements-skytemple-randomizer.json"))
            run_cargo_generator(
                os.path.join(path_skytemple_rust, "Cargo.lock"),
                os.path.join(out_dir, "cargo-sources-skytemple-rust.json"),
            )
            fixup_and_add_extra_pip_dependencies(os.path.join(out_dir, "requirements-skytemple-randomizer.json"), extras)
...
def parse_reqs_with_skytemple_files_reqs(req_file_path_skytemple: str) -> dict[str, Requirement]:
    """Concat the requirement files of SkyTemple Randomizer and SkyTemple Files"""
    print_t("Collecting requirements of SkyTemple Randomizer and its requested SkyTemple Files version")
...
def stable_tag() -> str:
    tag = get_pkgname_and_version_from_pyproject_toml(SKYTEMPLE_RANDOMIZER_PYPROJECT_FILE)[1]
    print_i(f"SkyTemple Randomizer Tag: {tag}")
    return tag
