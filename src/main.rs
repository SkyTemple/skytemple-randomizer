...
const APP_ID: &str = "org.skytemple.Randomizer";
    ...
    let exit_code = {
        ...
        match MainPythonInterpreter::new(config) {
            Ok(interp) => {
                ...
                app.connect_activate(move |app| build_ui(app, &interp));
...
fn build_ui(app: &Application, interp: &MainPythonInterpreter) {
    let text = interp.with_gil(|py| {
        let locals = PyDict::new(py);
        py.run(
            "import pydoc; import skytemple_files; ret = pydoc.render_doc(skytemple_files, renderer=pydoc.plaintext)",
            None,
            Some(locals),
        )
        .unwrap();
        let ret = locals.get_item("ret").unwrap();
        let ret_pystr: &PyString = ret.downcast().unwrap();
        ret_pystr.to_string()
    });
